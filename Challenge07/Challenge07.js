...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (o.yearsOfExperience <= 1 || (o.firstName == "null" || o.firstName == "") && (o.lastName ==
            "null" || o.lastName == "")) {
...
const classesAvg = (data) => {
    ...
    for (let i = 0; i < data.grades.length; i++) {
        ...
        for (let j = 0; j < grade.classes.length; j++) {
            const c = grade.classes[j];
            let sum = 0;

            for (let k = 0; k < c.classScores.length; k++)
                sum += c.classScores[k];
            c.avg = Math.floor(sum / c.classScores.length);
        }
    }
    return data;
};
